...
describe('Next.js Applications', () => {
    ...
    updateFile(
      ...
      `
        ...
        const DynamicComponent = dynamic(
            () => import('@${projName}/${libName}').then(d => d.${stringUtils.capitalize(
        libName
      )})
          );
      ` + readFile(mainPath)
  ...
  it('should compile when using a workspace and react lib written in TypeScript', async () => {
    ...
    updateFile(
      mainPath,
      `
        import { testFn } from '@${projName}/${tsLibName}';
        import { TestComponent } from '@${projName}/${tsxLibName}';\n\n
        ` +
        content.replace(
          `</h2>`,
          `</h2>
